...
class YoloTest(object):
    def __init__(self):
        ...
        self.__yolo_output = YoloV2().build_network(self.__yolo_input, self.__is_training)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', default='./data/original/car.jpg', type=str)
    parser.add_argument('--video_save_path', default='', type=str)
    parser.add_argument('--image_save_path', default='', type=str)
    ...
    if 'ckpt' not in str(model_file).split('.')[-1]:
        raise RuntimeError('The model file must end with .ckpt-...')
    ...
    if (not os.path.exists(video_path)) and (not os.path.exists(image_path)):
        raise RuntimeError('You must enter a valid path of video or image')

    T = YoloTest()
    if os.path.exists(video_path):
        cap = cv2.VideoCapture(video_path)
        out = False if video_save_path == '' else cv2.VideoWriter(video_save_path)
        while cap.isOpened():
            ret, frame = cap.read()
            if ret:
                image = T.detect(frame)
                cv2.imshow('video', image)
                cv2.waitKey(1)
                if out: out.write(image)
            else:
                break
        cap.release()
